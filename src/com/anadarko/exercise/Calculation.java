...
public class Calculation {
		...
		for (int i = 0; i < all_readings.size(); i++) {
			...
			if (reading1.getPlatform_name().equals(reading2.getPlatform_name())) {
				...
				if (count == 0) {
					...
					populate_output_readings = populate_output_readings(reading1.getPlatform_name(), "0", "0",
							min_value, convert_epoch_to_time(max_flight_time));
				} else {
					populate_output_readings = populate_output_readings(reading1.getPlatform_name(),
	...
	private OutputReadings populate_output_readings(String platform_name, String total_gap_time, String avg_gap_time,
			int min_value, String max_flight_time) {
		OutputReadings outputReading = new OutputReadings();
		...
		System.out.println(platform_name + ", " + total_gap_time + ", " + avg_gap_time + ", " + min_value + ", "
				+ max_flight_time);
